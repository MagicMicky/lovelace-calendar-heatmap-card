...
function adjustColorForTheme(hexColor) {
  ...
  try {
    ...
    console.warn(`Error adjusting color for theme: ${error}. Using original color.`);
    return hexColor;
  }
}
...
function isCurrentThemeDark() {
  try {
    ...
    if (bgColor) {
      ...
      if (parsedColor) {
        const luminance = calculateLuminance(parsedColor.r, parsedColor.g, parsedColor.b);
...
export function adjustColor(hex, factor) {
  ...
  const bgColor = parseColor(CSS_VARIABLES.cardBackground) || { r: isDarkTheme ? 30 : 240, g: isDarkTheme ? 30 : 240, b: isDarkTheme ? 30 : 240 };
